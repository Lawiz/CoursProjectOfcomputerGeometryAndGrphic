...
void Полет::Рисовать(CDC& dc,CRect& ОК){
	...
	W(0)=Скорость*cos(Угол*pi/180)*(Время+1);
	W(1)=Скорость*sin(Угол*pi/180)*(Время+1)-5*(Время+1)*(Время+1);
	W=K*W;
	p.x=W(0);
	p.y=W(1);
		dc.LineTo(p);
...
void Полет::Лететь(){
	Время+=0.2;
	Точки(0,ЧислоТочек)=Скорость*cos(Угол*pi/180)*Время;
	Точки(1,ЧислоТочек)=Скорость*sin(Угол*pi/180)*Время-5*Время*Время;
